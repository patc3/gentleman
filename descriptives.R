...
# helper fn: add p column to num or fac desc table using list of p
add_p_column_from_p_list <- function(tbl, ana)
{
  "
  input: tbl generated by some tbl_fn; ana list of p-values generated by some ana_fn
  return: tbl with added p column
  "
  ana |>
    unlist() |> 
    data.frame() |> 
...
get_desc_table <- function(df, vars, tbl_fn, group=NULL, ana_fn=NULL, ...)
{
  ...
  input:
    ...
    ana_fn is fn with (df, vars, group) arguments, returns named list of p values (match with Var)
    ... passed to ana_fn
  ...
  if(!is.null(group))
  {
    ...
    # combine
    tbl <- tbl |> left_join(tbl_grp, by="Var")
  }
